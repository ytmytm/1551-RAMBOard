; Note: additional comments were added based on automatic comparison with
;       All About Your 1541. Trust but verify!
...
;
; ADDITIONAL COMMENTS:
;   - Cross-references to 1541 ROM addresses (AAY1541) added where applicable
;   - 1551 uses TIA chip ($4000) instead of VIAs ($1800/$1C00 on 1541)
;   - 1551 runs at 2MHz (vs 1MHz on 1541), enabling on-the-fly GCR decoding
;   - Zero page addresses differ significantly from 1541 (see mapping guide)
;
;
...
; TCBM bus interrupt handler (1551-specific, no 1541 equivalent)
; The 1551 uses TCBM (TED Computer Bus Manager) interface instead of IEEE-488
; This routine handles commands from the Plus/4 computer

S_C022	LDA V_4000	; read data from computer (TIA port A)
	CMP V_4000	; port stable? (double-read to ensure valid data)
	BEQ B_C02D	; if yes, then jump
	JMP L_EABD	; otherwise back to main wait loop
	;
B_C02D	AND #$0F	; lowest 4 bits only (command code)
	ASL A		; * 2 (to index word table)
	TAY		; Y = A, make it an index
	LDA V_C046,Y	; look up address of subroutine (low byte)
	STA zp6E
	LDA V_C046 + 1,Y ; high byte
	STA zp6E+1	; $6E/$6F = indirect jump address
	LDA V_4002
	AND #$F7	; clear ACK bit (bit 3)
	STA V_4002	; ACK handshake to computer
...
S_C066 	LDA V_4002	; not a valid command
	...
	; 1541 equivalent: IEEE-488 ATN handling (around $E5xx)
	; TCBM uses different protocol: commands come as 4-bit codes via port $4000
	; Command codes: $40=TALK, $20=LISTEN, $5F=UNTALK, $3F=UNLISTEN
	;
B_C071	LDA V_4002	; TIA status register
	BMI B_C071	; wait for DAV (Data Valid) handshake from plus/4
	LDA V_4000	; read port data in from plus/4
	PHA		; save to stack
	LDA zp58	; check if previous data byte pending
	BPL B_C08B	; MSB not set? (no pending data)
	LDA #$00	; reset end flag
	STA zp95	; clear EOI flag
	LDA zp59        ; read last data byte (was saved for delayed processing)
	JSR B_C0F6	; process pending data byte
	LDA #$00
	STA zp58	; clear pending flag
B_C08B	PLA		; read port value back from stack
	TSX		; save stack pointer
	STX zp59+1	; to $5A (for error recovery)
	LDX #$00
	STA zp8F	; save command byte
	TAY		; backup A (command) in Y
	AND #$60	; isolate command bits (bits 5-6)
	CMP #$40	; talk? ($40 = TALK command)
	BEQ B_C0A8	; yes
	CMP #$20	; listen? ($20 = LISTEN command)
	BEQ B_C0C0	; yes
	LDY #$00	; otherwise DATA ($60) - not a device command
	LDX #$03
	STX zp6E        ; set blink rate (LED flash timing)
	SEI		; disable interrupt
	JMP L_E984	; handle data transfer
	;
; Handle TALK command
B_C0A8	STX zp5B        ; reset TALK flag (X=0)
	TYA             ; code in accu
	CMP #$5F	; UNTALK ? ($5F = UNTALK command)
	BEQ B_C0BC	; yes...
	STA zp5B        ; set TALK flag (save command byte)
	STX zp5C        ; reset LISTEN flag
L_C0B3	LDA #$20	; default secondary address ($20 = channel 0)
	STA zp7C	; save sec. address
	STA zp7D	; backup copy
	JMP L_C133	; finish handshake
	;
B_C0BC	LDA #$00	; UNTALK: clear flags
	BEQ B_C135	; always jump to finish

; Handle LISTEN command
B_C0C0	TYA		; read back A from Y
	CMP #$3F	; UNLISTEN? ($3F = UNLISTEN command)
	BNE B_C0CF	; no
	LDA #$FF	; set EOI flag
	STA zp95	; end of transmission
	LDX #$00
	STX zp5C	; reset flag for listen
	BEQ B_C0BC	; always jump

B_C0CF	STA zp5C	; must be LISTEN then (save command)
	STX zp5B	; clear TALK flag
	JMP L_C0B3	; set default secondary address
	;
...
; 1541 equivalent: Similar logic in IEEE-488 TALK handler (around $E5xx)
; This routine sends data from drive buffers to the Plus/4 via TCBM interface
;
B_C14E	LDA V_4002
	BMI B_C14E	; wait for DAV (Data Valid) handshake from plus/4
	LDA #$FF
	STA V_4003	; set TIA port A to all outputs (1551: TIA vs 1541: VIA)
	TSX		; save stack pointer
	STX zp59+1	; ... to $5A
	LDA zp7C	; read saved secondary address (channel number)
	CMP #$13	; 19 (max channel + 1)
	BCC B_C163	; smaller, then skip
	AND #$0F	; mask to valid channel range
B_C163	CMP #$0F	; command channel (15)?
	BNE B_C169	; no...
	LDA #$10	; if 15 then make it 16 (command channel mapping)
B_C169	TAX		; make an index from it
	SEC
	LDA V_022B,X	; logical index for channel (1541: $022B-$022D)
	BMI B_C176	; channel not open?
	AND #$0F
	STA zp7B	; buffer number
	TAX
	CLC
B_C176	BCS B_C17E	; channel open?
	LDX zp7B
	LDA zpF3,X	; channel status (1541: $F3 table)
	BMI B_C189	; error?
B_C17E	LDA #$0D	; CR (carriage return)
	STA V_4000	; send $0D to computer via TIA port A
	LDA #$0A	; LF (line feed)
	STA zp97
	BNE B_C1A7	; always jump
B_C189	LDA V_023E,X
...
; 1541 equivalent: $D005 (AAY1541: RO41D005.HTM)
; Initializes the drive: finds drive number, loads BAM, prepares disk status
; Called when computer sends "I" command via command channel

	JSR S_C2BB	; find drive number (1541: $C1D1)
	JSR S_D15E	; load BAM (Block Allocation Map) from disk
	JMP L_C27E	; prepare disk status message
	;
	; Read BAM from disk
	;
S_D12A	JSR S_EF7A
...
B_D148	TXA
	...
	;
	; Load BAM
	; 1541 equivalent: $D042 (AAY1541: RO41D042.HTM)
	; Loads Block Allocation Map from disk into memory buffer
	;
S_D15E	JSR S_EF3C	; prepare for BAM read
	JSR S_D465	; close all channels
	JSR S_D12A	; read BAM block from disk
	LDX zp78	; drive number
	LDA #$00
	STA V_0251,X	; reset flag for "BAM changed" (1541: $0251)
	JSR L_D6D9	; wait for disk operation completion
	LDA zp98	; buffer number
	ASL A		; * 2 (buffer pointer table index)
	TAX
	LDA #$02	; buffer pointer to $200 (BAM buffer base)
	STA zp9A,X	; set buffer pointer (1541: $9A table)
	LDA (zp9A,X)	; get format byte from buffer (1541: 'A' = 1541 format)
	LDX zp78	; drive number
	STA zp5D,X	; save format marker (1541: $5D)
	LDA #$00
	STA zp22,X	; flag for write protect (1541: $22)
	STA zp5F,X	; flag for read error (1541: $5F)
	;
	; Calculate blocks free
	; 1541 equivalent: Similar calculation in BAM handling
	; Counts free blocks across all tracks (except track 18 = directory)
	;
L_D185	JSR S_EDA5	; buffer address to $6C/$6D (BAM buffer pointer)
	LDY #$04	; begin at position 4 (skip header, start at track 1)
	LDA #$00	; initialize free block counter (lo byte)
	TAX		; hi byte = 0
B_D18D	CLC
	ADC (zp6C),Y	; add number of free blocks per track (BAM entry byte 0)
	BCC B_D193	; no overflow?
	INX		; X as hi-byte (increment on overflow)
B_D193	INY		; skip to next track's BAM entry
	INY		; (each track uses 4 bytes in BAM)
	INY
	INY		; plus 4
	CPY #$48	; track 18? (18 * 4 = $48)
	BEQ B_D193	; then skip (directory track, not counted)
	CPY #$90	; last track number? (35 * 4 = $8C, but check $90)
	BNE B_D18D	; no, continue counting
	PHA		; lo-byte
	TXA		; hi-byte
	LDX zp78	; drive number
	STA V_02FC,X	; hi-byte to $2FC (1541: $02FC)
	PLA		; lo-byte
	STA V_02FA,X	; to $2FA (1541: $02FA)
	RTS
;$D1AB  --------
...
B_EA74	STA zpA8,X
	...
	;
	; Main wait loop
	; 1541 equivalent: Similar main loop (around $EAxx)
	; This is the idle loop that waits for commands from the computer
	; Checks for TCBM bus activity and processes commands when received
	;
L_EABD	INC zp61	; increment idle counter (for timing)
	BNE B_EAC4	; if not zero, skip periodic maintenance
	JSR S_EADC	; periodic maintenance (check channels, LED control)
B_EAC4	LDA V_4000	; read I/O port from computer (TIA port A)
	BPL B_EACC	; 7th bit set? (TCBM command pending)
	JSR S_C022	; yes, then handle TCBM command
B_EACC	LDA V_0255	; waiting for command? (command flag set)
	BEQ L_EABD	; no, continue idle loop
	LDA #$00	; yes, command received
	STA V_0255	; clear command flag
	JSR S_C230	; analyze and execute command (parse command string)
...
S_F388	LDA zp27
	...
	STA zp27
	RTS
;$F3BB  -------- Encode header to GCR
; 1541 equivalent: $F934 (AAY1541: RO41F934.HTM)
; Encodes 5-byte header block (ID1, ID2, Track, Sector, Checksum) to 8 GCR bytes
; 1551: Result stored at $0111 (1541: $0024)
; Uses S_F80C to encode 4 binary bytes to 5 GCR bytes
;
S_F3BB	LDA zp27+1
	STA zp1E+1
	LDA #$01
	...
	STA zp27+1
	RTS
;$F3F0  -------- Clear error flag
; 1541 equivalent: Similar error flag handling
; Clears error flag (V_026C) and optionally sets LED flash mode
;
S_F3F0	LDA #$00
	STA V_026C	; clear error flag (1541: $026C)
	BEQ B_F3F9	; always branch
	;
	; Set LED flag (flash mode for error indication)
	; 1541 equivalent: Similar LED control
	; Sets LED to flash mode ($E0) to indicate error condition
	;
L_F3F7	LDA #$E0	; LED flash mode ($E0 = flash on error)
	; Set LED flag to accu
B_F3F9	STA zp76	; drive LED flag for IRQ routine (1541: similar flag)
	RTS
;$F3FC  -------- Sector read/write dispatcher
; 1541 equivalent: Similar sector operation dispatcher
; Routes to read sector (B_F403) or write sector (L_F57D) based on job type
;
L_F3FC	CMP #$00	; job type: 0 = read, non-zero = write
	BEQ B_F403	; read sector
	JMP L_F57D	; write sector
	;
; Read sector entry point (job type = 0)
B_F403	JSR S_F513	; wait for header and sector sync
B_F406	BIT zp01
	BPL B_F406
	LDA V_4001
	STA zpFA
...
B_F44C	BIT zp01
	...
	INY
	BEQ B_F4D9
; Continue decoding: process remaining 3 bytes from 5-byte GCR group
; Byte #2: uses bits from GCR bytes #2 and #3
B_F464	LDA zpFB	; GCR byte #2 (from previous iteration)
	AND #$3E	; isolate middle 5 bits (bits 1-5)
	TAX
	LDA V_F70C,X	; decode upper nybble of byte #2
	STA zpF9
	LDA zpFB
	AND #$01	; isolate lowest bit (will combine with byte #3)
	STA zpFB
B_F474	BIT zp01	; wait for GCR byte #3
	BPL B_F474
	LDA V_4001
	STA zpFC
	AND #$F0	; upper 4 bits
	ORA zpFB	; combine with previous bit = 5-bit index
	TAX
	LDA V_F70C + 2,X	; decode (note: +2 offset for this table access)
	ORA zpF9	; combine = binary byte #2
	STA (zp27),Y	; write to buffer
	INY
	LDA zpFC
	AND #$0F	; lower 4 bits (will combine with byte #4)
	STA zpFC
; Byte #3: uses bits from GCR bytes #3 and #4
B_F490	BIT zp01	; wait for GCR byte #4
	BPL B_F490
	LDA V_4001
	STA zpFD
	AND #$80	; upper bit
	ORA zpFC	; combine with previous 4 bits = 5-bit index
	TAX
	LDA V_F71C,X	; decode using different table offset
	STA zpF9	; upper nybble of byte #3
	LDA zpFD
	AND #$7C	; middle 5 bits (bits 2-6)
	TAX
	LDA V_F70C + 1,X	; decode (note: +1 offset)
	ORA zpF9	; combine = binary byte #3
	STA (zp27),Y	; write to buffer
	INY
	LDA zpFD
	AND #$03	; lower 2 bits (will combine with byte #5)
	STA zpFD
; Byte #4: uses bits from GCR bytes #4 and #5
B_F4B6	BIT zp01	; wait for GCR byte #5 (last of group)
	BPL B_F4B6
	LDA V_4001
	STA zpFA
	AND #$E0	; upper 3 bits
	ORA zpFD	; combine with previous 2 bits = 5-bit index
	TAX
	LDA V_F729,X	; decode using different table offset
	STA zpF9	; upper nybble of byte #4
	LDA zpFA
	AND #$1F	; lower 5 bits
	TAX
	LDA V_F6FF,X	; decode (note: V_F6FF is base of overlapping tables)
	ORA zpF9	; combine = binary byte #4
	STA (zp27),Y	; write to buffer
	INY
	JMP B_F435	; loop back to process next 5-byte GCR group
	;
B_F4D9	LDA zpFB
	AND #$3E
	TAX
...
B_F50E	LDA #$01
	JMP L_FA12
	;
; $F513: Wait for data block (header sync + sector sync)
; 1541 equivalent: $F50A (AAY1541: RO41F50A.HTM)
; Waits for the correct header block, then waits for sector data sync
;
S_F513	JSR S_F519	; encode header and wait for header match
	JMP L_F560	; then wait for sector sync
	;
; $F519: Encode header and wait for header
; 1541 equivalent: $F510 (AAY1541: RO41F510.HTM)
; Prepares header block (track, sector, ID, checksum) and waits for matching header
; 1551 differences: Header encoded to $0111 (vs $0024 on 1541)
;                   Uses zp14/zp15 for disk ID (vs $12/$13 on 1541)
;                   Uses zp20 for track/sector pointer (vs $32 on 1541)
;
S_F519	LDA zp14	; disk ID byte 1 (1541: $12)
	STA zp18	; header ID 1 (1541: $16)
	LDA zp15	; disk ID byte 2 (1541: $13)
	STA zp19	; header ID 2 (1541: $17)
	LDY #$00
	LDA (zp20),Y	; track number (1541: ($32),Y)
	STA zp1A	; header track (1541: $18)
	INY
	LDA (zp20),Y	; sector number (1541: ($32)+1,Y)
	STA zp1B	; header sector (1541: $19)
	LDA #$00
	EOR zp18	; calculate header checksum
	EOR zp19
	EOR zp1A
	EOR zp1B
	STA zp1C	; header checksum (1541: $1A)
	JSR S_F3BB	; encode header to GCR (1541: $F934, result to $0111 vs $0024)
	LDA #$5A	; retry counter (1541: LDX #$5A)
	STA zp3C	; 1551 uses zp3C instead of X register
B_F53F	JSR L_F560	; wait for sync (1541: $F556)
B_F542	LDA V_0111,Y	; compare encoded header (1541: $0024,Y)
B_F545	BIT zp01	; wait for byte ready
	BPL B_F545
	CMP V_4001	; compare with disk (1541: $1C01)
	BNE B_F554	; mismatch, retry
	INY
	CPY #$08	; all 8 GCR bytes matched?
	BNE B_F542
	RTS		; header found!
;$F554  --------
B_F554	DEC zp3C
...
L_F55B	LDA #$03
	JMP L_FA12
	;
; $F560: Wait for SYNC mark
; 1541 equivalent: $F556 (AAY1541: RO41F556.HTM)
; Waits for SYNC mark (gap between blocks). On 1551, SYNC is bit 6 of $4002 (0=found)
; On 1541, SYNC is bit 7 of $1C00 (1=found), so logic is inverted
;
L_F560	LDY #$12	; outer loop counter
B_F562	LDX #$FF	; inner loop counter
B_F564	LDA V_4002	; TIA status register (1541: $1C00 VIA)
	AND #$40	; check SYNC bit (bit 6)
	BEQ B_F574	; SYNC found! (0 = found on 1551, vs 1 = found on 1541)
	DEX
	BNE B_F564	; inner loop
	DEY
	BNE B_F562	; outer loop
	JMP L_F55B	; timeout error
	;
B_F574	LDA V_4001	; read first byte after SYNC (clear latch)
	BIT V_4000	; clear byte-ready flag (1551: BIT $4000, 1541: CLV)
	LDY #$00	; set Y=0 for subsequent reads
	RTS
;$F57D  --------
L_F57D	CMP #$10
	BEQ B_F584
...
; $F809
	...
	; 1541 equivalent: $F6D0 (AAY1541: RO41F6D0.HTM)
	; Encodes 4 binary bytes (from zp41-zp44) into 5 GCR bytes
	; Output written to (zp27),Y starting at offset zp2D
	; Uses lookup table V_F8BF for GCR encoding
	;
S_F80C	LDA #$00
	STA zp45+1	; temp storage
	STA zp49	; temp storage
	LDY zp2D	; output offset (1541: $34)
	LDA zp41	; first binary byte
	...
	LDA V_F8BF,X	; GCR encoding table (1541: similar table)
	ASL A
	ASL A
	ASL A		; times 8
	STA zp45
...
; 1541 equivalent: Similar table used for GCR encoding
; Maps 4-bit binary nybbles to 5-bit GCR codes
; Used by S_F80C to encode binary data to GCR format
;
V_F8BF	DB $0A,$0B,$12,$13,$0E,$0F,$16,$17
	DB $09,$19,$1A,$1B,$0D,$1D,$1E,$15
	;
	; Convert 260 bytes to 325 bytes group code
	; 1541 equivalent: Similar GCR encoding routine
	; Converts 260-byte sector (256 data + 4 overhead) to 325 GCR bytes
	; Used when writing sectors to disk
	;
	LDA #$00
	STA zp1E
	STA zp2E
...
B_F91B	LDA zp30
	...
	; 1541 equivalent: $F7E6 (AAY1541: RO41F7E6.HTM)
	; Decodes 5 GCR-encoded bytes into 4 binary bytes using lookup tables
	; 1551 uses overlapping tables at $F6FF-$F7FF (vs separate pages in 1571)
	; Input: 5 GCR bytes from (zp27),Y starting at offset zp2D
	; Output: 4 binary bytes in zp41-zp44 (1541: $52-$55)
	;
S_F928	LDY zp2D	; GCR offset (1541: $34)
	LDA (zp27),Y
	TAX
	AND #$F8
	STA zp45
...
B_FA0D	LDA zp1E+1
	STA zp27+1
	RTS
;$FA12  -------- Error termination / Return to main loop
; 1541 equivalent: $F969 (AAY1541: RO41F969.HTM)
; Returns from disk controller routine with error code in accumulator
; Sets error flag, restores stack, returns to main IRQ handler
;
L_FA12	LDY zp32+1	; buffer number (1541: $3F)
	STA $0002,Y	; store error code in job buffer (1541: $00,Y)
	LDA zp3F	; GCR/BIN indicator (1541: $50)
	BEQ B_FA1E	; skip if binary mode
	JSR S_F618	; decode GCR buffer (1541: $F5F2)
B_FA1E	JSR S_FA36	; set LED flag (1541: $F98F)
	LDX zp3A	; restore stack pointer (1541: $49)
	TXS
	JMP L_F19D	; return to IRQ handler (1541: $F2BE)
	;
S_FA27	LDA #$A0
	STA zp2B
	LDA zp01
...
;
; ============================================================================
; ADDITIONAL NOTES FOR PORTING FROM 1541 TO 1551:
; ============================================================================
;
; KEY DIFFERENCES:
; 1. Hardware: TIA chip ($4000-$4005) replaces VIAs ($1800/$1C00)
; 2. Bus: TCBM interface replaces IEEE-488 serial bus
; 3. Speed: 2MHz CPU enables on-the-fly GCR decoding (like 1571)
; 4. Zero Page: Many addresses shifted (see doc/1551-mapping-guide.txt)
;
; IMPORTANT FUNCTION MAPPINGS:
;   Read Sector:    1541 $F4D1  -> 1551 $F403
;   Wait for Sync:  1541 $F556  -> 1551 $F560
;   Calculate Parity: 1541 $F5E9 -> 1551 $F60A
;   Initialize:    1541 $D005  -> 1551 $D121
;   Error Handler: 1541 $F969  -> 1551 $FA12
;   GCR Decode:     1541 $F7E6  -> 1551 $F928
;   Encode Header:  1541 $F934  -> 1551 $F3BB
;
; GCR DECODING TABLES:
;   1551 uses overlapping tables at $F6FF-$F7FF (single page)
;   1571 uses separate pages ($9F0D, $A00D, $A10D, $A20D, $A30D)
;   Both achieve same result, 1551 saves ROM space
;
; For detailed mapping information, see:
;   - doc/1551-mapping-guide.txt
;   - 1541-1551-mapping.txt
;
